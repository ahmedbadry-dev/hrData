export { default as HomeHeroSection } from './HomeHeroSection/HomeHeroSection';
export { default as HomeHowSection } from './HomeHowSection/HomeHowSection';
export { default as HomeFeaturesSection } from './HomeFeaturesSection/HomeFeaturesSection';
export { default as HomeQuoteSection } from './HomeQuoteSection/HomeQuoteSection';
export { default as HomeGoogleDataSection } from './HomeGoogleDataSection/HomeGoogleDataSection';
export { default as HomeCtaSection } from './HomeCtaSection/HomeCtaSection';
export { default as HomeFooterSection } from './HomeFooterSection/HomeFooterSection';
export { default as HomeHowItWorksSection } from './HomeHowItWorksSection/HomeHowItWorksSection';
