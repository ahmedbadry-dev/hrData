...
/**
 * معالجة وظيفة واحدة: جلب المحتوى وبدء التنفيذ (الـ AI معلق حالياً بناءً على طلب المستخدم)
 */
async function processSingleJob(jobUrl: string, site: SiteConfig): Promise<string | null> {
  try {
    ...
    // ─────────────────────────────────────────────────────────────
    // الجزء المعلق حالياً (يمكن تفعيله لاحقاً بفك الـ comments)
    // ─────────────────────────────────────────────────────────────
...
/**
 * الوظيفة الرئيسية: تشغيل السكرابر لجميع المواقع المحددة
 */
  ...
  try {
    for (const site of SITES_CONFIG) {
      try {
        ...
        // تجميع الروابط لملف allAds.json
        allLinks.push(...jobLinks.map((url) => ({ site: site.name, url })));

        // معالجة الروابط بالتوازي مع احترام الحد الأقصى للمهام المتزامنة
        await Promise.all(
          jobLinks.map((jobUrl) =>
            limit(async () => {
              const content = await processSingleJob(jobUrl, site);
              
              // تجميع مادة للوظيفة لملف job.json (بحد أقصى 20 وظيفة إجمالاً)
    ...
    // تصدير البيانات النهائية للملفات
