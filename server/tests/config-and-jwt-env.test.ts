...
const withRequiredBaseEnv = () => {
  process.env.DATABASE_URL =
    process.env.DATABASE_URL ?? 'postgresql://user:pass@localhost:5432/kafoo';
  process.env.ENCRYPTION_KEY =
    process.env.ENCRYPTION_KEY ??
    '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
  process.env.JWT_ACCESS_SECRET =
    process.env.JWT_ACCESS_SECRET ?? 'access-secret-for-tests-0123456789abcdef';
  process.env.JWT_REFRESH_SECRET =
    process.env.JWT_REFRESH_SECRET ?? 'refresh-secret-for-tests-0123456789abcdef';
  process.env.JWT_VERIFICATION_TEMP_SECRET =
    process.env.JWT_VERIFICATION_TEMP_SECRET ?? 'verification-secret-for-tests-0123456789';
};
...
test('env config reads redis host from REDIS_HOST', async () => {
  ...
  const envModule = await import(`../src/config/env.config.ts?redis-host-${Date.now()}`);
